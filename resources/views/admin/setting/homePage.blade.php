<div class="container mx-auto px-4 py-8">
    ...
    <form class="bg-white dark:bg-gray-800 p-8 shadow-lg rounded-lg space-y-6" action="{{ route('setting.store') }}" method="POST">
        ...
        <div>
            <label for="sliderCategoryCard"
                class="block text-sm font-medium text-gray-700 dark:text-gray-300 capitalize">slider Category
                Card</label>
                <x-select id="sliderCategoryCard" name="sliderCategoryCard" :data="$data" :selectedData="$settings" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-100" > </x-select>
                @error('sliderCategoryCard')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
